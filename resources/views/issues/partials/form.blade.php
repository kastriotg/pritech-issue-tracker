@php
    $selectedTagIds = collect(old('tag_ids', isset($issue) ? $issue->tags->pluck('id')->all() : []))
    ...
    @else
        <select id="project_id" name="project_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            @foreach ($projects as $project)
                <option value="{{ $project->id }}" @selected((int) old('project_id', $issue->project_id ?? 0) === $project->id)>
                    {{ $project->name }}
                </option>
            @endforeach
        </select>
...
<div>
    ...
    <textarea id="description" name="description" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $issue->description ?? '') }}</textarea>
...
<div class="grid gap-6 sm:grid-cols-3">
    <div>
        ...
        <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            <option value="open" @selected(old('status', $issue->status ?? 'open') === 'open')>{{ __('Open') }}</option>
            <option value="in_progress" @selected(old('status', $issue->status ?? '') === 'in_progress')>{{ __('In Progress') }}</option>
            <option value="closed" @selected(old('status', $issue->status ?? '') === 'closed')>{{ __('Closed') }}</option>
        </select>
    ...
    <div>
        ...
        <select id="priority" name="priority" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            <option value="medium" @selected(old('priority', $issue->priority ?? 'medium') === 'medium')>{{ __('Medium') }}</option>
    ...
    <div>
        ...
        <x-text-input id="due_date" name="due_date" type="date" class="mt-1 block w-full" :value="old('due_date', isset($issue) ? $issue->due_date?->toDateString() : '')" />
    ...
    <div class="mt-2 flex flex-wrap gap-2">
        @forelse ($tags as $tag)
            <label class="inline-flex cursor-pointer items-center gap-2 rounded-full border px-3 py-2 text-sm font-medium transition hover:bg-gray-50">
                <input type="checkbox" name="tag_ids[]" value="{{ $tag->id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(in_array($tag->id, $selectedTagIds, true))>
                <span
